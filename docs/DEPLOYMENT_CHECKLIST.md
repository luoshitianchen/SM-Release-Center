# 发布中心服务 上线部署检查清单（DEPLOYMENT_CHECKLIST）
...
- [ ] ConfigMap 环境变量正确（SM_ENV / SM_DATABASE_NAME=sm_release_center）
...
1. `helm lint helm/sm-release-center`
2. `helm upgrade --install sm-release-center helm/sm-release-center -n sm-prod -f helm/sm-release-center/values-prod.yaml`
3. 等待 rollout：`kubectl -n sm-prod rollout status deploy/sm-release-center`
